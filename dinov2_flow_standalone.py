...
这是一个经过完整实验验证的配方（MVTec-AD 15 类 image AUROC 0.984，ViT-B/14）：
  ...
  - 抽第 2/5/8/11 个 block 的原始输出（pre-LN、不做最终归一化），通道维拼接
    后用 1×1 卷积压到 512 维
  - FastFlow 2D 头（8 个仿射耦合，log-scale 软限幅 ±2，分组转置置换）建模
    正常分布，异常图 = 每位置 NLL
...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        return outs                             # list[(B,C,H,W)]，由 flow 头拼接压缩


# ---------------------------------------------------------------- flow 头
# ---- 以下 flow 头与验证版（MVTec 0.984 配方）逐字一致，改动会破坏稳定性 ----
def subnet_conv(ch_in, ch_out):
    return nn.Sequential(
        nn.Conv2d(ch_in, ch_in // 2, 3, padding=1), nn.ReLU(inplace=True),
        nn.Conv2d(ch_in // 2, ch_out, 3, padding=1))


class AffineCoupling2D(nn.Module):
    """2D 仿射耦合。log-scale 软限幅 ±2（tanh）是数值稳定的关键，勿改。"""

    def __init__(self, channels, clamp=2.0):
        super().__init__()
        self.clamp = clamp
        self.split_len = channels // 2
        self.subnet = subnet_conv(channels // 2, channels)
    ...
    def forward(self, x):
        ...
        z2 = x2 * torch.exp(s) + t
        return torch.cat([x1, z2], dim=1), s.flatten(1).sum(1)


class FlowBlock2D(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.flow = AffineCoupling2D(channels)
        self.channels = channels

    def _shuffle(self, x, groups=8):           # 固定分组转置，作层间置换
        b, c, h, w = x.shape
        return x.reshape(b, groups, c // groups, h, w).transpose(1, 2).reshape(b, c, h, w)

    def forward(self, x):
        z, ldj = self.flow(x)
        return self._shuffle(z), ldj


class FastFlow2D(nn.Module):
    """多层特征拼到最细网格 → 1×1 压缩到 flow_channels → 8 个耦合块。

    NLL 只对前半通道计算（与验证版一致）。feats: list[(B,C_i,H_i,W_i)]。
    """

    def __init__(self, in_channels, flow_channels=512, n_coupling=8):
        super().__init__()
        total = sum(in_channels)
        self.compress = nn.Conv2d(total, flow_channels, 1)
        self.blocks = nn.ModuleList([FlowBlock2D(flow_channels) for _ in range(n_coupling)])
        self.register_buffer("prior_mean", torch.zeros(flow_channels // 2), persistent=False)
        self.register_buffer("prior_std", torch.ones(flow_channels // 2), persistent=False)

    def forward(self, feats):
        f0 = feats[0]
        ups = [f0] + [F.interpolate(f, size=f0.shape[-2:], mode="bilinear",
                                    align_corners=False) for f in feats[1:]]
        x = self.compress(torch.cat(ups, dim=1))
        ...
        z = x[:, :x.shape[1] // 2]
        c = z.shape[1]
        log_p = -0.5 * (c * torch.log(torch.tensor(2 * torch.pi, device=z.device))
                        + (z ** 2 / self.prior_std.view(1, -1, 1, 1) ** 2).sum(1))
        return -(log_p + logdet.view(-1, 1, 1))     # (B,H,W) NLL 图，越高越异常
...
def run_model(backbone, flow, images, device, bs=16):
    ...
    for i in range(0, len(images), bs):
        ...
        feats = backbone(x)                     # list[(B,C,H,W)]
        nll = flow(feats)                       # (B,H,W) 每位置 NLL
...
def cmd_train(args):
    ...
    embed_dim = backbone.model.embed_dim if hasattr(backbone.model, "embed_dim") \
        else backbone(torch.zeros(1, 3, args.size, args.size, device=device))[0].shape[1]
    flow = FastFlow2D(in_channels=[embed_dim] * len(OUT_INDICES),
                      flow_channels=args.flow_channels).to(device)
...
def cfg_dict(args, embed_dim):
    return {"size": args.size, "embed_dim": embed_dim,
            "out_indices": list(OUT_INDICES), "flow_channels": args.flow_channels,
...
def load_flow(exp_dir, device, source="final.pt"):
    ...
    flow = FastFlow2D(in_channels=[cfg["embed_dim"]] * len(cfg["out_indices"]),
                      flow_channels=cfg["flow_channels"]).to(device)
...
def main():
    ...
    t.add_argument("--flow-channels", type=int, default=512,
                   help="flow 头压缩后的通道数（验证配方为 512）")
